...
def test_async_tuple_modal_operation_is_guarded(tmp_path: Path) -> None:
    workspace = tmp_path
    script = workspace / "TRR-Backend" / "scripts" / "unsafe_async.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        "async def deploy():\n    command = ('modal', 'deploy', '-m', 'trr_backend.modal_jobs')\n    return command\n",
        encoding="utf-8",
    )
    allowlist = workspace / "allowed.json"
    allowlist.write_text(json.dumps({"version": 1, "allowed": []}), encoding="utf-8")

    violations = guard.find_unapproved_invocations(workspace_root=workspace, allowlist_path=allowlist)

    assert [(item.function, item.operation) for item in violations] == [("deploy", "deploy")]
