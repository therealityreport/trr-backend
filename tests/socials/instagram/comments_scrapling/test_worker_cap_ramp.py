...
def test_launch_config_defaults_to_12_for_public_runs():
    cfg = _instagram_comments_worker_cap_launch_config(
        public_mode=True,
        requested_comments_worker_count=None,
    )
    assert cfg["comments_worker_cap_current"] == 12
    assert cfg["comments_worker_cap_floor"] == 6
    assert cfg["comments_worker_cap_start"] == 12
    assert cfg["comments_worker_cap_steps"] == [15, 20]
    assert cfg["comments_worker_cap_ceiling"] == 20
    assert cfg["comments_worker_cap_pause_reason"] is None
    assert cfg["comments_worker_cap_history"] == []


def test_launch_config_honors_smaller_explicit_worker_count():
    cfg = _instagram_comments_worker_cap_launch_config(
        public_mode=True,
        requested_comments_worker_count=8,
    )
    assert cfg["comments_worker_cap_current"] == 8
...
def test_launch_config_does_not_start_above_start_for_large_request():
    ...
    # A larger explicit request never lifts the *starting* cap above 12; the ramp
...
def test_normalize_fills_defaults_for_partial_config():
    ...
    assert normalized["current"] == 15
    assert normalized["floor"] == 6
    assert normalized["ceiling"] == 20
    assert normalized["steps"] == [15, 20]
...
def test_orchestrator_ramps_up_persists_history_and_refills(monkeypatch: pytest.MonkeyPatch):
    run_config = {
        "comments_worker_cap_current": 12,
        "comments_worker_cap_floor": 6,
        "comments_worker_cap_start": 12,
        "comments_worker_cap_steps": [15, 20],
        "comments_worker_cap_ceiling": 20,
...
def test_orchestrator_ramps_down_to_floor_on_high_ratio(monkeypatch: pytest.MonkeyPatch):
    run_config = {
        "comments_worker_cap_current": 20,
        "comments_worker_cap_floor": 6,
        "comments_worker_cap_start": 12,
        "comments_worker_cap_steps": [15, 20],
        "comments_worker_cap_ceiling": 20,
    ...
    assert result["cap"] == 6
    updates = calls["merge"][0]["metadata_updates"]
    assert updates["comments_worker_cap_current"] == 6
