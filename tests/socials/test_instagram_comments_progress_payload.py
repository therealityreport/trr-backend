...
def test_public_recovery_launches_public_relay_run(monkeypatch) -> None:
    ...
    assert merged[0]["metadata_updates"]["public_recovery"]["source_run_id"] == (
...
def test_audit_cursor_recovery_show_filter_uses_saved_post_metadata(monkeypatch) -> None:
    ...
    assert datetime(2026, 1, 1, tzinfo=timezone.utc) in params
    assert datetime(2027, 1, 1, tzinfo=timezone.utc) in params
