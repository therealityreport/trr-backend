...
def test_fetch_all_with_logo_variant_fallback_retries_with_null_variants() -> None:
    ...
    with patch("api.routers.admin_brands.pg.fetch_all", side_effect=_fake_fetch_all):
        rows = admin_brands._fetch_all_with_logo_variant_fallback(
            ...
            params=[],
            variant_sql=(
                "hosted_logo_black_url as hosted_logo_black_url, "
                "hosted_logo_white_url as hosted_logo_white_url,"
            ),
...
def test_list_brand_logos_include_missing_adds_synthetic_role_rows() -> None:
    ...
    with patch("api.routers.admin_brands.pg.fetch_all", return_value=existing_rows), patch(
        "api.routers.admin_brands.pg.fetch_one", return_value={"count": 1}
    ), patch(
...
def test_list_brand_logos_include_related_ignores_missing_related_variant_columns() -> None:
    ...
    with patch("api.routers.admin_brands.pg.fetch_all", return_value=base_rows), patch(
...
def test_list_logo_option_sources_groups_counts() -> None:
    ...
    with patch("api.routers.admin_brands._list_logo_options", return_value={"rows": rows, "count": 3}):
...
def test_discover_logo_candidates_filters_existing_without_dropping_nonmatching_roles() -> None:
    ...
    with patch("api.routers.admin_brands._list_brand_logos", return_value={"rows": existing_rows, "count": 1}), patch(
