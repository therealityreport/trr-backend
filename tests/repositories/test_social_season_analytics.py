...
def test_get_week_detail_fetches_platforms_concurrently(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(social_repo, "_resolve_week_run_row", lambda *args, **kwargs: None)
...
def test_get_week_detail_overrides_zero_post_status_with_active_run_jobs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        social_repo,
        "_resolve_week_run_row",
        lambda *args, **kwargs: {"run_id": "run-live", "status": "running"},
    )
    monkeypatch.setattr(
        social_repo,
...
def test_ingest_instagram_details_refresh_preserves_views_when_detail_has_no_view_candidate(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _context, **kwargs: (
            enqueued_post_ids.append(str((kwargs.get("post_row") or {}).get("id") or ""))
            or "mirror-job-1"
        ),
    )
                ...
                "items": [
                    {
                        ...
                        "video": {
                            ...
                            "snippet": {
                                ...
                                "channelTitle": "Bravo",
                                "thumbnails": {
                                    "high": {"url": "https://img.youtube.com/vi/vid-api-1/hqdefault.jpg"}
                                },
                                "tags": ["RHOSLC"],
                            },
