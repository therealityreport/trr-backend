"""Compatibility boundary for the Instagram catalog-ingest legacy owner transition."""

from __future__ import annotations

import ast
import os
import subprocess
import sys
from pathlib import Path

import pytest

import trr_backend.repositories.social_season_analytics as legacy_repo
import trr_backend.socials.control_plane.run_lifecycle as run_lifecycle
import trr_backend.socials.instagram.catalog_ingest as catalog_ingest
import trr_backend.socials.social_season_analytics_impl as legacy_impl

BACKEND_ROOT = Path(__file__).resolve().parents[2]
CATALOG_INGEST_PATH = BACKEND_ROOT / "trr_backend" / "socials" / "instagram" / "catalog_ingest.py"
LEGACY_MODULES = {
    "trr_backend.repositories.social_season_analytics",
    "trr_backend.socials.social_season_analytics_impl",
}
FRESH_IMPORT_ORDERS = (
    "trr_backend.socials.instagram.catalog_ingest",
    "trr_backend.socials.control_plane.run_lifecycle",
    "trr_backend.socials.control_plane",
    "trr_backend.socials.social_season_analytics_impl",
    "trr_backend.repositories.social_season_analytics",
    "trr_backend.socials.instagram.persistence",
    "trr_backend.socials.instagram.posts_scrapling.persistence",
)


def _run_fresh_process(*lines: str) -> subprocess.CompletedProcess[str]:
    environment = dict(os.environ)
    environment["PYTHONDONTWRITEBYTECODE"] = "1"
    return subprocess.run(
        [sys.executable, "-B", "-c", "\n".join(lines)],
        cwd=BACKEND_ROOT,
        env=environment,
        capture_output=True,
        text=True,
        check=False,
    )


def test_catalog_ingest_owns_exact_monolith_module_through_lifecycle_loader() -> None:
    assert catalog_ingest._core is legacy_impl
    assert legacy_repo is legacy_impl
    assert legacy_repo.__dict__ is legacy_impl.__dict__
    assert run_lifecycle._legacy_module() is legacy_impl


def test_catalog_ingest_observes_late_repository_patch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sentinel = object()
    monkeypatch.setattr(legacy_repo, "_r3_catalog_ingest_probe", sentinel, raising=False)

    assert catalog_ingest._core._r3_catalog_ingest_probe is sentinel
    assert "_r3_catalog_ingest_probe" not in catalog_ingest.__dict__


def test_catalog_ingest_uses_late_provider_publication_without_a_legacy_bootstrap_import() -> None:
    source = CATALOG_INGEST_PATH.read_text()
    tree = ast.parse(source)
    direct_legacy_imports = [
        node
        for node in ast.walk(tree)
        if (isinstance(node, ast.ImportFrom) and node.module in LEGACY_MODULES)
        or (isinstance(node, ast.Import) and any(alias.name in LEGACY_MODULES for alias in node.names))
    ]
    lifecycle_imports = [
        alias
        for node in tree.body
        if isinstance(node, ast.ImportFrom) and node.module == "trr_backend.socials.control_plane.run_lifecycle"
        for alias in node.names
        if (alias.name, alias.asname) == ("_legacy_module", "_published_legacy_module")
    ]
    registry_imports = [
        alias.name
        for node in tree.body
        if isinstance(node, ast.ImportFrom) and node.module == "trr_backend.socials.provider_registry"
        for alias in node.names
    ]
    deleted_names = {
        target.id
        for node in tree.body
        if isinstance(node, ast.Delete)
        for target in node.targets
        if isinstance(target, ast.Name)
    }
    dynamic_imports = [
        node
        for node in ast.walk(tree)
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and isinstance(node.func.value, ast.Name)
        and node.func.value.id == "importlib"
        and node.func.attr == "import_module"
    ]

    assert direct_legacy_imports == []
    assert len(lifecycle_imports) == 1
    assert {"LateNamespaceProvider", "LateProviderProxy", "adopt_published"}.issubset(registry_imports)
    assert "_published_legacy_module" in deleted_names
    assert dynamic_imports == []
    assert "_published_legacy_module" not in catalog_ingest.__dict__


@pytest.mark.parametrize("primary_module", FRESH_IMPORT_ORDERS)
def test_fresh_import_orders_preserve_exact_identity_and_room_contracts(
    primary_module: str,
) -> None:
    result = _run_fresh_process(
        "import importlib",
        f"importlib.import_module({primary_module!r})",
        "catalog = importlib.import_module('trr_backend.socials.instagram.catalog_ingest')",
        "lifecycle = importlib.import_module('trr_backend.socials.control_plane.run_lifecycle')",
        "impl = importlib.import_module('trr_backend.socials.social_season_analytics_impl')",
        "repo = importlib.import_module('trr_backend.repositories.social_season_analytics')",
        "assert catalog._core is impl",
        "assert lifecycle._legacy_module() is impl",
        "assert repo is impl",
        "assert repo.__dict__ is impl.__dict__",
        "assert '_published_legacy_module' not in catalog.__dict__",
        "local = catalog._LOCAL_ROOM_FUNCTIONS",
        "assert local",
        "assert set(local) == catalog._LOCAL_ROOM_NAMES",
        "sentinel = object()",
        "repo._r3_catalog_ingest_fresh_probe = sentinel",
        "assert catalog._core._r3_catalog_ingest_fresh_probe is sentinel",
        "assert '_r3_catalog_ingest_fresh_probe' not in catalog.__dict__",
    )

    assert result.returncode == 0, result.stderr


def test_repository_patch_before_catalog_ingest_reload_stays_visible() -> None:
    result = _run_fresh_process(
        "import importlib",
        "repo = importlib.import_module('trr_backend.repositories.social_season_analytics')",
        "sentinel = object()",
        "repo._r3_catalog_ingest_reload_probe = sentinel",
        "catalog = importlib.import_module('trr_backend.socials.instagram.catalog_ingest')",
        "catalog = importlib.reload(catalog)",
        "impl = importlib.import_module('trr_backend.socials.social_season_analytics_impl')",
        "assert catalog._core is impl",
        "assert catalog._core._r3_catalog_ingest_reload_probe is sentinel",
        "assert '_published_legacy_module' not in catalog.__dict__",
    )

    assert result.returncode == 0, result.stderr
