...
from trr_backend.socials.control_plane import queue_status, run_reads


def test_run_reads_ordinary_import_defers_provider_until_late_publication() -> None:
    ...
    script = "\n".join(
        (
            ...
            "import trr_backend.socials.control_plane.run_reads as leaf",
...
def test_run_reads_reuses_exact_queue_status_provider() -> None:
    assert run_reads.legacy is queue_status._legacy_repo()
    assert queue_status._LEGACY_NAMESPACE is legacy_core.__dict__
    assert run_reads.legacy.__name__ == legacy_core.__name__


def test_run_reads_source_keeps_legacy_access_function_scoped() -> None:
    source_path = Path(run_reads.__file__).resolve()
...
def test_run_reads_provider_fails_deterministically_when_unconfigured(
    ...
    with monkeypatch.context() as scoped:
        ...
        with pytest.raises(
            ...
            _ = run_reads.legacy.pg


def test_run_reads_provider_observes_late_legacy_monkeypatch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with monkeypatch.context() as scoped:
        ...
        assert run_reads.legacy._get_social_hot_path_cache is replacement


def test_run_reads_provider_assignment_writes_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
