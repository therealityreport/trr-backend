...
def test_default_renders_all_excluded() -> None:
    ...
    assert clause == (
        "text = EXCLUDED.text, likes = EXCLUDED.likes, author_full_name = EXCLUDED.author_full_name"
    )
...
def test_preserve_columns_render_coalesce_and_others_render_excluded() -> None:
    ...
    assert (
        "author_full_name = COALESCE(EXCLUDED.author_full_name, social.instagram_comments.author_full_name)"
        in clause
    )
    assert (
        "author_profile_pic_url = "
        "COALESCE(EXCLUDED.author_profile_pic_url, social.instagram_comments.author_profile_pic_url)"
        in clause
    )
...
def test_preserve_set_uses_provided_table_name() -> None:
    ...
    assert clause == (
        "author_full_name = COALESCE(EXCLUDED.author_full_name, social.twitter_tweets.author_full_name)"
    )
...
def test_instagram_preserve_constant_matches_persistence_constant() -> None:
    ...
    assert (
        persistence._INSTAGRAM_COMMENT_COALESCE_PRESERVE_COLS
        == social_repo._INSTAGRAM_COMMENT_COALESCE_PRESERVE_COLS
    )
