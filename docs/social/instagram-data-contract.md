...
| --- | --- | --- |
| `profile_timeline_xdt` | `trr_backend/socials/instagram/posts_scrapling/fetcher.py`, `trr_backend/socials/instagram/scraper.py`, `trr_backend/repositories/social_season_analytics.py` | `xdt_api__v1__feed__user_timeline_graphql_connection`, `PolarisProfilePostsQuery`; form/runtime tags: `fb_api_caller_class`, `fb_api_req_friendly_name`, `variables`, `server_timestamps`, `doc_id`, `av`, `__d`, `__user`, `__a`, `__req`, `__comet_req`, `lsd`, `__spin_r`, `__spin_b`, `__spin_t`, `hsi`; headers: `x-fb-friendly-name`, `x-fb-lsd`, `x-asbd-id`, `x-bloks-version-id`, `x-root-field-name`; response paths: `data.xdt_api__v1__feed__user_timeline_graphql_connection.edges`, `.page_info`, `.count`. |
...
| `profile_following_list` | `trr_backend/repositories/social_season_analytics.py` shared job stage `instagram_profile_following`, `trr_backend/socials/instagram/profile_stages.py` | `https://www.instagram.com/api/v1/friendships/{profile_id}/following/`; profile id comes from the web profile GraphQL shape, then the REST request uses `count`, `max_id`, same-origin headers, and optional authenticated web header env values (`SOCIAL_INSTAGRAM_IG_WWW_CLAIM`, `SOCIAL_INSTAGRAM_WEB_SESSION_ID`). Response paths: `users[]`, `next_max_id`, `big_list`/`has_more`. Persist only when the requested mode and source row both mean `following`. |
| `socialblade_following_sidecar` | `trr_backend/socials/socialblade/service.py`, `trr_backend/modal_jobs.py` | SocialBlade worker wrapper runs `instagram_profile_following` after Instagram SocialBlade refreshes. The sidecar is nonfatal and records `instagram_following_scrape` status in the persisted SocialBlade raw response. Complete runs also create immutable following snapshot rows; partial runs are evidence of observed follows only and must not infer unfollows. |
