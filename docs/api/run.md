...
| `TRR_PREVIEW_READ_ONLY` | Isolated-preview opt-in. When `1`, every backend pool checkout and direct psycopg2 connection preserves existing URI options, adds `-c default_transaction_read_only=on`, then actively executes `SET default_transaction_read_only = on` and requires `SHOW transaction_read_only = on` before use; otherwise startup or the caller fails closed. The temporary autocommit mode is restored before the caller receives the connection. PostgREST cache reloads are refused. The API also skips broker/background-task startup; at Modal import/deploy the flag is explicitly forwarded to the API and credential-free browser-runtime canary, and Modal forces `SOCIAL_QUEUE_ENABLED=false`. Leave `0` for normal runtimes. | `0` / `1` |
