...
def _find_media_asset_id_by_hosted_sha(db: Any, *, source: str | None, hosted_sha256: str | None) -> str | None:
    clean_source = str(source or "").strip()
    clean_sha = str(hosted_sha256 or "").strip()
    if not clean_source or not clean_sha:
        return None
    response = (
        db.schema("core")
        .table("media_assets")
        .select("id")
        .eq("source", clean_source)
        .eq("hosted_sha256", clean_sha)
        .limit(1)
        .execute()
    )
...
def _add_run_history_to_metadata(metadata: dict[str, Any], run_id: str | None) -> dict[str, Any]:
    clean_run_id = str(run_id or "").strip()
    if not clean_run_id:
        return metadata
    run_ids: list[str] = []
    for value in _safe_list(metadata.get("bravotv_run_ids")):
        text = str(value or "").strip()
        if text and text not in run_ids:
            run_ids.append(text)
    existing_run_id = str(metadata.get("run_id") or "").strip()
    if existing_run_id and existing_run_id not in run_ids:
        run_ids.append(existing_run_id)
    if clean_run_id not in run_ids:
        run_ids.append(clean_run_id)
    metadata["bravotv_run_ids"] = run_ids
    metadata["bravotv_last_run_id"] = clean_run_id
    return metadata


def _record_reused_asset_run_attribution(db: Any, *, asset_id: str, asset_payload: dict[str, Any]) -> None:
    incoming_metadata = _safe_json(asset_payload.get("metadata"))
    run_id = str(incoming_metadata.get("run_id") or "").strip()
    if not run_id:
        return
    try:
        ...
        db.schema("core").table("media_assets").update({"metadata": updated_metadata}).eq("id", asset_id).execute()
    except Exception as exc:  # noqa: BLE001
        logger.warning("Failed to record BRAVOTV reused asset run attribution for %s: %s", asset_id, exc)


def _upsert_media_asset_for_import(db: Any, asset_payload: dict[str, Any]) -> tuple[str, bool]:
    try:
        rows = upsert_media_assets(db, [asset_payload])
    except RuntimeError as exc:
        if "media_assets_source_hosted_sha_uq" not in str(exc):
            raise
        existing_asset_id = _find_media_asset_id_by_hosted_sha(
            db,
            source=str(asset_payload.get("source") or "").strip() or None,
            hosted_sha256=str(asset_payload.get("hosted_sha256") or "").strip() or None,
        )
        if not existing_asset_id:
            raise
        _record_reused_asset_run_attribution(db, asset_id=existing_asset_id, asset_payload=asset_payload)
        return existing_asset_id, False
    returned = rows[0] if rows and isinstance(rows[0], dict) else {}
    asset_id = str(returned.get("id") or asset_payload.get("id") or "").strip()
    if not asset_id:
        raise RuntimeError("BRAVOTV media asset upsert did not return an asset id.")
...
def _import_catalog(
    *,
    ...
    for record in merged_catalog:
        ...
        asset_id, asset_was_upserted = _upsert_media_asset_for_import(db, asset_payload)
        if asset_was_upserted:
            assets_upserted += 1
        if not preview_only:
            try:
        ...
        imported_records.append(
            {
                ...
                "source_asset_id": asset_payload.get("source_asset_id"),
                "source_url": asset_payload.get("source_url"),
                "source_page_url": _safe_json(asset_payload.get("metadata")).get("source_page_url"),
                "hosted_url": asset_payload.get("hosted_url"),
                "hosted_key": asset_payload.get("hosted_key"),
                "hosted_sha256": asset_payload.get("hosted_sha256"),
                "asset_reused": not asset_was_upserted,
                "caption": record.get("caption"),
                "show_name": show_name,
    ...
    *,
    ...
    for source, rows in supplemental_catalog.items():
        ...
        for row in rows:
            ...
            asset_id, asset_was_upserted = _upsert_media_asset_for_import(db, payload)
            if asset_was_upserted:
                assets_upserted += 1
            ...
            if link_episode and episode_id:
                ...
                links_created += 1
                link_targets.append(f"episode:{episode_id}")
            imported.append(
                {
                    "media_asset_id": str(asset_id),
                    "source": source,
                    "source_asset_id": source_asset_id,
                    "source_url": source_url,
                    "source_page_url": asset_metadata.get("source_page_url"),
                    "hosted_url": raw_hosted.get("hosted_url"),
                    "hosted_key": raw_hosted.get("hosted_key"),
                    "hosted_sha256": raw_hosted.get("hosted_sha256"),
                    "asset_reused": not asset_was_upserted,
...
def _build_cast_source_links_export(
    *,
    run_id: str,
    mode: str,
    ...
    for record in imported_records:
        ...
        hosted_url = str(record.get("hosted_url") or "").strip() or None
        source_url = str(record.get("source_url") or "").strip() or None
        if source not in sources:
            sources[source] = {"count": 0, "r2_count": 0, "examples": []}
        ...
        if isinstance(examples, list) and len(examples) < 10:
            examples.append(
                {
                    "media_asset_id": record.get("media_asset_id"),
                    "r2_url": hosted_url,
                    "source_url": source_url,
                    "source_page_url": record.get("source_page_url"),
                    "caption": record.get("caption"),
                    "asset_reused": bool(record.get("asset_reused")),
                    "supplemental": bool(record.get("supplemental")),
                    "link_targets": _safe_list(record.get("link_targets")),
                }
            )
    return {
        "run_id": run_id,
...
def execute_bravotv_image_run(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f"bravotv-run-{run_id[:8]}-") as temp_dir:
        ...
        cast_source_links = _build_cast_source_links_export(
            run_id=run_id,
            mode=mode,
            person_name=person_name,
            show_name=show_name,
        ...
        (output_root / "cast_source_links.json").write_text(
            json.dumps(cast_source_links, indent=2, ensure_ascii=True, default=_json_default) + "\n"
        )
        run_review = build_run_review_from_dir(output_root)
        (output_root / "run_review.json").write_text(
            json.dumps(run_review, indent=2, ensure_ascii=True, default=_json_default) + "\n"
        )
        ...
        summary["review_reason_counts"] = run_review.get("review_reason_counts") or {}
        summary["duplicate_groups"] = (run_review.get("summary") or {}).get("total_duplicate_groups", 0)
        run = update_progress(
            run_id,
            status="completed",
    ...
    complete_payload = {
        ...
        "artifacts": {
            ...
            "cast_source_links": _ARTIFACT_PREVIEW_LIMIT,
