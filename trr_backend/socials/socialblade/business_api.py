...
class SocialBladeBusinessApiUnavailable(RuntimeError):
...
def _build_payload(handle: str, platform: str, raw_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    media_count = int(_first_number(totals.get("media"), totals.get("uploads"), totals.get("posts"), profile.get("media")))
...
def scrape_socialblade_business_api(handle: str, *, platform: str = "instagram") -> dict[str, Any]:
    ...
    with httpx.Client(timeout=max(5.0, timeout_seconds)) as client:
        response = client.get(f"{base_url}/{normalized_platform}/statistics", headers=headers, params={"query": safe_handle})
