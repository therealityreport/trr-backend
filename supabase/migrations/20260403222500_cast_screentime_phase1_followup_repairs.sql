...
alter table if exists ml.analysis_media_upload_sessions
  add column if not exists verified_size_bytes bigint,
  add column if not exists verified_checksum_sha256 text,
  add column if not exists verified_at timestamptz,
  add column if not exists failed_at timestamptz,
  add column if not exists promoted_video_asset_id uuid references ml.analysis_media_assets(id) on delete set null,
  add column if not exists error_text text;

do $$
begin
  if to_regclass('screenalytics.video_assets') is null then
    raise notice 'screenalytics.video_assets not present; skipping cast screentime Phase 1 legacy asset backfill repair';
    return;
  end if;
