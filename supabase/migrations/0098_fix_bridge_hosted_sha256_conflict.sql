-- Fix cast_photos bridge: handle hosted_sha256 unique constraint violations
-- The previous fix (0094) handled source_url conflicts but the INSERT statements
-- using ON CONFLICT (source, source_asset_id) and ON CONFLICT (source, source_url)
-- don't handle hosted_sha256 conflicts, causing unique constraint violations.
...
begin
  ...
  -- First lookup: try to find existing media_asset by any matching key
  ...
  -- If we found an existing media_asset, update it and create the link
  if v_media_asset_id is not null then
    -- Check for source_url conflict before updating
    ...
    -- Check for hosted_sha256 conflict before updating
    ...
    -- No existing media_asset found, try to insert
    -- Use a single INSERT with ON CONFLICT DO NOTHING to handle all unique constraints
    -- Then look up the record if the insert was a no-op due to conflict
    ...
    -- If insert returned nothing (conflict occurred), look up the existing record
    if v_media_asset_id is null then
      select id into v_media_asset_id
      from core.media_assets
      where source = new.source
        and (
          (v_hosted_sha is not null and hosted_sha256 = v_hosted_sha)
          or (new.source_image_id is not null and source_asset_id = new.source_image_id)
          or (v_source_url is not null and source_url = v_source_url)
        )
      order by created_at asc
      limit 1;
    end if;
  ...
  -- If we still don't have a media_asset_id, we can't create the link
  if v_media_asset_id is null then
    return new;
  end if;

  -- Create or update the media_link
