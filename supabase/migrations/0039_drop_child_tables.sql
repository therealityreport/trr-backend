...
-- =============================================================================
-- Migration 0039: Drop deprecated child tables
-- =============================================================================
-- IMPORTANT: Only apply this migration AFTER:
--   1. Migration 0037 has backfilled array columns from child tables
--   2. Python code has been updated to write directly to core.shows
--   3. Ingestion has been tested successfully
--
-- This migration:
--   1. Drops all child attribute tables
--   2. Drops tmdb_series_external_ids (data now in core.shows)
--   3. Updates merge_shows to remove child table cascade logic
...
-- ---------------------------------------------------------------------------
-- 2. Drop child attribute tables
-- ---------------------------------------------------------------------------

drop table if exists core.show_genres cascade;
drop table if exists core.show_keywords cascade;
drop table if exists core.show_tags cascade;
drop table if exists core.show_networks cascade;
drop table if exists core.show_streaming_providers cascade;

-- ---------------------------------------------------------------------------
-- 3. Drop tmdb_series_external_ids (data migrated to core.shows)
-- ---------------------------------------------------------------------------

drop table if exists core.tmdb_series_external_ids cascade;

-- ---------------------------------------------------------------------------
-- 4. Update merge_shows to remove child table cascade logic
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- ---------------------------------------------------------------------------
  -- Cascade to remaining child tables
  -- ---------------------------------------------------------------------------
