    ...
    if not approved:
        lines.extend(
            [
                ...
                "-- Add approved_to_drop=yes and the required approval fields in owner packet CSVs, then regenerate this file.",
                "",
            ]
        )
        return "\n".join(lines)
...
def render_markdown_report(rows: list[dict[str, Any]], *, metadata: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Generated by `TRR-Backend/scripts/db/unused_index_evidence_report.py`. This report contains no database credentials.",
        "",
        ...
        *_table(
        ...
        "- `drop_review_required` means live stats show `idx_scan = 0`, the snapshot named the index, and default exclusions did not apply.",
        "- `approved_to_drop` is always `no` until a route owner records reason, rollback SQL, and a 7-day production recheck or explicit urgent approval.",
        "- Constraint-backed, primary, unique, exclusion, FK-hardening, and recent-migration indexes are excluded by default.",
        "",
    ]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate an unused-index evidence review from live pg_stat_user_indexes.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.approval_packet_dir:
        if not args.drop_sql_output:
            print("[unused-index-evidence] ERROR: --drop-sql-output is required with --approval-packet-dir", file=sys.stderr)
            return 2
    ...
    if args.drop_sql_output and not args.owner_packet_dir:
        print("[unused-index-evidence] ERROR: --drop-sql-output requires --owner-packet-dir or --approval-packet-dir", file=sys.stderr)
        return 2
