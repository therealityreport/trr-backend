...
def sync_single_show(
    ...
    *,
    ...
    steps = [
        ("show_images", ["scripts/sync_show_images.py", "--imdb-id", imdb_id], False),
        ("seasons", ["scripts/sync_seasons.py", "--imdb-id", imdb_id], False),
        ("season_images", ["scripts/sync_season_episode_images.py", "--imdb-id", imdb_id], False),
    ]

    if not skip_imdb:
        steps.extend(
            [
                ("episodes", ["scripts/sync_episodes.py", "--imdb-id", imdb_id], True),  # IMDb
                ("show_cast", ["scripts/sync_show_cast.py", "--imdb-id", imdb_id], True),  # IMDb
                ("episode_appearances", ["scripts/sync_episode_appearances.py", "--imdb-id", imdb_id], True),  # IMDb
            ]
        )

    if not skip_images:
        steps.append(("cast_photos", ["scripts/sync_cast_photos.py", "--imdb-show-id", imdb_id], False))
