...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="sync_seasons",
        description="Sync core.seasons from TMDb for existing shows.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not show_rows:
        print("No shows need seasons sync.")
        return 0
    ...
    for show in show_rows:
        ...
        if not show_id:
            continue
        if not args.dry_run:
            mark_sync_state_in_progress(db, table_name="seasons", show_id=show_id)
