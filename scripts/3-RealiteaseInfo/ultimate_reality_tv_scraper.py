...
def get_realitease_value(row, field):
    """Safely get a value from a spreadsheet row using column mapping"""
    index = REALITEASE_COLUMN_INDEX[field]
    return row[index] if len(row) > index else ''


def column_number_to_letter(column_number):
    """Convert a 1-based column number into its spreadsheet column letter"""
    result = ''
...
def normalize_name(name: str) -> str:
    """Normalize names for comparison (lowercase, accent-free, trimmed)."""
    if not name:
        return ''
...
def parse_date_text_to_yyyy_mm_dd(text: str) -> Optional[str]:
    ...
    # Year-first with optional month/day (YYYY-MM-DD, YYYY-MM)
    ...
    # Numeric formats like MM-DD-YYYY or MM-DD-YY
...
class FamousBirthdaysEnhancer:
    ...
    def configure_gemini(self):
        """Configure the Google Gemini API if possible."""
        ...
        if not genai:
            print("⚠️ google-generativeai package not installed; skipping Gemini integration")
            return
    ...
    def analyze_text_for_gender(self, text, cast_name=""):
        """Determine gender from biography text, preferring Gemini for classification."""
        if not text:
            return None
    ...
    def _analyze_text_for_gender_pronouns(self, text, cast_name=""):
        """Fallback pronoun- and keyword-based gender detection."""
    ...
    def process_cast_member(self, row_data, row_num):
        ...
        if needs_gender or needs_birthday:
            ...
            # Try sources in order of reliability (temporarily start with Gemini)
            ...
            for source_name, source_func in sources_to_try:
                ...
                if result:
                    ...
                    if source_name == 'fandom_wiki' and result.get('gender') and result.get('bio'):
                        # Record that fandom provided its own text-based gender
                        self.last_gender_analysis_source = 'text_analysis_fallback'
    ...
    def search_gemini(self, cast_name, show_name):
        """Use Google Gemini to look up birthday and gender."""
        ...
        try:
            ...
            if '404' in message and 'models/' in message:
                print("      ℹ️ Hint: Update GOOGLE_GEMINI_MODEL (e.g., 'gemini-1.5-flash' or 'gemini-2.0-flash') to match available models")
            return None

    def search_tmdb_cast(self, cast_name, show_tmdb_ids, cast_tmdb_id=''):
        """Use TMDb person ID when available, otherwise fall back to aggregate credits search."""
    ...
    def search_google(self, cast_name, show_names):
        """Google search disabled in current configuration."""
        return None
    ...
    def log_cast_summary(self, row_data, field_sources, updates_made):
        """Log a concise summary showing data values and their origins"""
    ...
    def format_gender_for_log(self, gender_value):
        """Return a display-friendly gender string"""
        if not gender_value:
            return 'UNKNOWN'
    ...
    def format_birthday_for_log(self, birthday_value):
        """Format birthday as MM-DD-YYYY for logging"""
        ...
        # Try already formatted MM-DD-YYYY
        try:
    ...
    def describe_source_label(self, source_key):
        """Translate internal source key to a human-readable label"""
        source_map = {
