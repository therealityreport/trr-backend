...
def extract_first_last(name: str) -> Tuple[str, str]:
    ...
    first = parts[0]
    last = parts[-1]
    # For hyphenated names we keep both the full token and last segment
...
def names_match(a: str, b: str) -> bool:
    ...
    if last_a == last_b:
        return True
    # Allow hyphenated variants (e.g., "Smith-Jones" vs "Jones")
...
class RealiteaseTMDbCastBackfiller:
    ...
    def process_rows(self, start_row: Optional[int], end_row: Optional[int], limit: Optional[int], reverse: bool = False) -> None:
        ...
        indexed_rows: List[Tuple[int, List[str]]] = [
            (idx + 1, row) for idx, row in enumerate(all_rows)
        ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Backfill TMDb cast IDs in RealiteaseInfo by TMDb show cast lookups.',
    )
