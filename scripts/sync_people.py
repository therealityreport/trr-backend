...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="sync_people",
        description="Sync core.people (cast members) from IMDb full credits (Self only).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for show in show_rows:
        ...
        if not show_id or not imdb_id:
            if args.verbose:
                print(f"SKIP show id={show.get('id')} (missing imdb_series_id)")
            continue
        if not args.dry_run:
        ...
        try:
            ...
            if missing_ids:
                ...
                if new_people_rows and not args.dry_run:
                    inserted_rows = insert_people(db, new_people_rows)
                    people_inserted += len(inserted_rows)
                    for person in inserted_rows:
                        imdb_value = str((person.get("external_ids") or {}).get("imdb") or "").strip().lower()
                        if imdb_value:
                            people_cache[imdb_value] = str(person.get("id"))
