...
def main(argv: list[str] | None = None) -> int:
    ...
    if not show_response.data:
        ...
        print("Import the show first with: PYTHONPATH=. python scripts/sync/sync_shows.py --imdb-id {imdb_id}")
        return 1
    ...
    if not args.skip_images:
        success, error = run_command(
            _build_cmd(["scripts/sync/sync_show_images.py", "--imdb-id", imdb_id], args.verbose),
    ...
    success, error = run_command(
        _build_cmd(["scripts/sync/sync_seasons.py", "--imdb-id", imdb_id], args.verbose),
        "Seasons metadata",
        args.verbose,
    )
    ...
    if not args.skip_images:
        success, error = run_command(
            _build_cmd(["scripts/sync/sync_season_episode_images.py", "--imdb-id", imdb_id], args.verbose),
    ...
    success, error = run_command(
        _build_cmd(["scripts/sync/sync_episodes.py", "--imdb-id", imdb_id], args.verbose),
        "Episodes metadata",
        args.verbose,
    )
    ...
    success, error = run_command(
        _build_cmd(["scripts/sync/sync_show_cast.py", "--imdb-id", imdb_id], args.verbose),
        "Show cast (main cast)",
        args.verbose,
    )
    ...
    success, error = run_command(
        _build_cmd(["scripts/sync/sync_episode_appearances.py", "--imdb-id", imdb_id], args.verbose),
    ...
    if not args.skip_images:
        ...
        success, error = run_command(
            _build_cmd(["scripts/sync/sync_cast_photos.py", "--imdb-show-id", imdb_id], args.verbose),
