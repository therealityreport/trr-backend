...
Usage:
    ...
    PYTHONPATH=. python scripts/sync/sync_cast_photos.py --imdb-person-id nm11883948

    # Sync only TMDb and Fandom sources
    PYTHONPATH=. python scripts/sync/sync_cast_photos.py --imdb-person-id nm11883948 --source tmdb --source fandom

    # Sync without S3 mirroring
    PYTHONPATH=. python scripts/sync/sync_cast_photos.py --imdb-person-id nm11883948 --no-s3

    # Sync without pruning orphaned S3 objects
    PYTHONPATH=. python scripts/sync/sync_cast_photos.py --imdb-person-id nm11883948 --no-prune

    # Dry run (no mutations)
    PYTHONPATH=. python scripts/sync/sync_cast_photos.py --imdb-person-id nm11883948 --dry-run
"""
