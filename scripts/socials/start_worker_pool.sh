...
#   SOCIAL_WORKER_POOL_SHARED_ACCOUNT_DISCOVERY=1
...
SHARED_ACCOUNT_DISCOVERY_WORKERS="${SOCIAL_WORKER_POOL_SHARED_ACCOUNT_DISCOVERY:-1}"
...
SHARED_ACCOUNT_DISCOVERY_WORKERS="$(normalize_count "$SHARED_ACCOUNT_DISCOVERY_WORKERS")"
...
SHARED_ACCOUNT_DISCOVERY_WORKERS="$(apply_stage_floor "$SHARED_ACCOUNT_DISCOVERY_WORKERS")"
...
if [[ "$POSTS_WORKERS" -eq 0 && "$COMMENTS_WORKERS" -eq 0 && "$SHARED_ACCOUNT_DISCOVERY_WORKERS" -eq 0 && "$SHARED_ACCOUNT_POSTS_WORKERS" -eq 0 && "$POST_CLASSIFY_WORKERS" -eq 0 && "$SEASON_MATERIALIZE_WORKERS" -eq 0 && "$ANALYTICS_REFRESH_WORKERS" -eq 0 && "$MEDIA_MIRROR_WORKERS" -eq 0 && "$COMMENT_MEDIA_MIRROR_WORKERS" -eq 0 ]]; then
...
if [[ "$SHARED_ACCOUNT_DISCOVERY_WORKERS" -gt 0 ]]; then
  start_worker "shared_account_discovery" --stage shared_account_discovery --parallel "$SHARED_ACCOUNT_DISCOVERY_WORKERS" --interval "$WORKER_INTERVAL"
fi
...
echo "[social-worker-pool] started (posts=${POSTS_WORKERS}, comments=${COMMENTS_WORKERS}, shared_account_discovery=${SHARED_ACCOUNT_DISCOVERY_WORKERS}, shared_account_posts=${SHARED_ACCOUNT_POSTS_WORKERS}, post_classify=${POST_CLASSIFY_WORKERS}, season_materialize=${SEASON_MATERIALIZE_WORKERS}, analytics_refresh=${ANALYTICS_REFRESH_WORKERS}, media_mirror=${MEDIA_MIRROR_WORKERS}, comment_media_mirror=${COMMENT_MEDIA_MIRROR_WORKERS}, min_stage_runners=${MIN_STAGE_RUNNERS}, allow_stage_disable=${ALLOW_STAGE_DISABLE})"
wait
