...
def _ensure_repair_proxy_defaults() -> dict[str, str]:
    """Route the repair validation scrape through the Decodo residential proxy by default.

    Cloudflare 1020 is an IP/fingerprint verdict, so the surest way to make the local
    validation succeed is to fetch from a residential egress. ``setdefault`` respects an
    explicit env value, so ``SOCIALBLADE_PROXY_PROVIDER=`` (empty) forces the unproxied /
    visible-browser path for testing.
    """
    os.environ.setdefault("SOCIALBLADE_PROXY_PROVIDER", "decodo")
    os.environ.setdefault("SOCIALBLADE_USE_STICKY_PROXY", "true")
    return {
