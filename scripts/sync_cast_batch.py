...
def sync_cast_for_show(
    ...
    *,
    ...
    success, error = run_command(
        ["scripts/sync_show_cast.py", "--imdb-id", imdb_id] + (["--verbose"] if verbose else []),
        "Show cast (main cast)",
        verbose,
    )
    ...
    success, error = run_command(
        ["scripts/sync_episode_appearances.py", "--imdb-id", imdb_id] + (["--verbose"] if verbose else []),
    ...
    if not skip_cast_photos:
        success, error = run_command(
            ["scripts/sync_cast_photos.py", "--imdb-show-id", imdb_id] + (["--verbose"] if verbose else []),
